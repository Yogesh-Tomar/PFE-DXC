...
public class PlanActionController {
    ...
    public List<PlanActionDTO> getAll() {
    	...
    	List<PlanActionDTO> pdt= planActionService.getAll();
    	System.out.println("=============>controll planres"+pdt.toString());
        return planActionService.getAll();
    }

    @GetMapping("/{id}")
    public PlanActionDTO getById(@PathVariable Long id) {
        return planActionService.getByIddto(id);
    }
