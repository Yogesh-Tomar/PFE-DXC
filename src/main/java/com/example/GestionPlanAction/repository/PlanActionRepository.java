...
public interface PlanActionRepository extends JpaRepository<PlanAction, Long> {
    ...
    @Query(value = "SELECT p.* , v.* ,e.* ,u.* FROM plan_action p left join variable_action v on v.plan_action_id=p.id left join exercice e on e.id=p.exercice_id left join user u on u.id=v.responsable_id",nativeQuery = true)
